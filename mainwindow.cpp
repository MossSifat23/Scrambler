    ...
    , ui(new Ui::MainWindow)
{
    ...
    //ui->label->text() to get the actual text back nah not needed
        ...
        ui->progressBar->setValue(2); // change back to totalSeconds

        timer1 = new QTimer(this);
        connect(timer1, &QTimer::timeout, this, &MainWindow::ontimeout) ;

        firestoreUrls= {
            "https://firestore.googleapis.com/v1/projects/wordle-m-c/databases/(default)/documents/Users/Top",
            "https://firestore.googleapis.com/v1/projects/wordle-m-c/databases/(default)/documents/Users/2",
            "https://firestore.googleapis.com/v1/projects/wordle-m-c/databases/(default)/documents/Users/3",
            "https://firestore.googleapis.com/v1/projects/wordle-m-c/databases/(default)/documents/Users/4",
            "https://firestore.googleapis.com/v1/projects/wordle-m-c/databases/(default)/documents/Users/5"
        };
...
void MainWindow::readFromFirestore()
{

  //  QUrl url(QString(
     //            "https://firestore.googleapis.com/v1/projects/%1/databases/(default)/documents/%2/%3"
       //          ).arg(projectId, collection, documentId));
    QNetworkRequest request(firestoreUrls[0]);
    ...
    QObject::connect(mgr, &QNetworkAccessManager::finished,
                     [=](QNetworkReply* reply)
                     {
                         ...
                         qDebug() << "RAW Firestore response:" << response;

                         show_leader(response);   // 👈 call your print function
...
void MainWindow:: show_leader(const QByteArray& data)
{
    ...
    if (!root.contains("fields")) {
        qDebug() << "No fields found!";
        return;
    }

    QJsonObject fields = root["fields"].toObject();

    // Example fields inside your document
    QString name = fields["name"].toObject()["stringValue"].toString();
    int score = fields["score"].toObject()["integerValue"].toString().toInt();

    ui->U1 -> setText(name);
    ui->N1 ->setText(QString::number(score));
}
...
void MainWindow::on_pushButton_2_clicked() //Lv.1 Next Button
{
    ...
    {
        if(currentRow==3 && win==current_lvl)
        {
            timer2->stop();
            ui->stackedWidget->setCurrentIndex(5); //Game Over
        }
...
void MainWindow::on_pushButton_7_clicked() //Game Over & Retry
{
    ui->stackedWidget->setCurrentIndex(6); //To Game Start Page
}
